...
const defaultState: AppState = {
  ...
  firebase: { databaseURL:'', secretPath:'631128' }, workerUrl:'', autoSync:false, autoSyncSec:60
};

function normalizeState(raw: any): AppState {
  const v = { ...defaultState, ...(raw || {}) };
  return { ...v, holdings: Array.isArray(v.holdings) ? v.holdings : defaultState.holdings, cash: Array.isArray(v.cash) ? v.cash : defaultState.cash, loans: Array.isArray(v.loans) ? v.loans : defaultState.loans, trades: Array.isArray(v.trades) ? v.trades : [], firebase: { ...defaultState.firebase, ...(v.firebase || {}) }, workerUrl: String(v.workerUrl || ''), refreshSec: Math.max(15, num(v.refreshSec || 60)), autoSync: Boolean(v.autoSync), autoSyncSec: Math.max(10, num(v.autoSyncSec || 60)) };
}
...
function writeState(s: AppState) { localStorage.setItem(STORAGE_KEY, JSON.stringify(s)); localStorage.setItem(OLD_STORAGE_KEY, JSON.stringify(s)); }
...
async function uploadFirebase(config: FirebaseConfig, state: AppState) { const res = await fetch(syncUrl(config), { method:'PUT', headers:{'content-type':'application/json'}, body:JSON.stringify(state) }); if (!res.ok) throw new Error(`Firebase ${res.status}`); }
...
function parseWorkerQuote(symbol: SymbolCode, data: any): Quote | null {
  if (typeof data?.price === 'number') { const prev = Number(data.previousClose ?? data.prev ?? data.price); return { ...defaultQuotes[symbol], price:data.price, previousClose:prev, change:data.price-prev, changePct: prev ? (data.price-prev)/prev*100 : 0, volume:Number(data.volume ?? 0), source:data.source ?? 'Cloudflare Worker', updatedAt: now() }; }
  const meta = data?.chart?.result?.[0]?.meta;
  if (meta?.regularMarketPrice) { const p = Number(meta.regularMarketPrice); const prev = Number(meta.previousClose || meta.chartPreviousClose || p); return { ...defaultQuotes[symbol], price:p, previousClose:prev, change:p-prev, changePct: prev ? (p-prev)/prev*100 : 0, volume:Number(meta.regularMarketVolume || 0), source:'Yahoo via Worker', updatedAt: now() }; }
  return null;
}
async function fetchQuote(symbol: SymbolCode, workerUrl: string): Promise<Quote> {
  ...
  for (const url of urls) { try { const res = await fetch(url, { cache:'no-store' }); if (!res.ok) continue; const q = parseWorkerQuote(symbol, await res.json()); if (q) return q; } catch {} }
...
function App(){
  ...
  const uploadCloud=async(label='已上傳')=>{ setSync('上傳中…'); await uploadFirebase(state.firebase,state); setSync(`${label} ${tw(now())}｜${syncPath(state.firebase)}｜持股 ${state.holdings.length} 筆｜交易 ${state.trades.length} 筆`); };
  const downloadCloud=async()=>{ setSync('下載中…'); const remote=await downloadFirebase(state.firebase); setState(remote); setSync(`已下載 ${tw(now())}｜${syncPath(state.firebase)}｜持股 ${remote.holdings.length} 筆｜交易 ${remote.trades.length} 筆`); };
